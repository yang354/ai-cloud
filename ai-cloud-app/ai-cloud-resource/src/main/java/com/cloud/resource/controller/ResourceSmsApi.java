...
import com.cloud.redis.utils.RedisLock;
...
public class ResourceSmsApi {
    ...
    public R<?> smsCaptcha(@RequestBody PhoneParam param,
                           @RequestHeader(value = LangConstants.LANGUAGE, required = false)
                           @ApiParam(value = "语言类型", example = "zh-CN") String language) {
        if (!smsProperties.getEnabled()) {
            return R.fail("当前系统没有开启短信功能");
        }
        System.out.println(SpringUtils.getBean(RedisLock.class).tryLock("aaa",10000l)+"~~~~~~~~~");
